...
export async function middleware(request: NextRequest) {
  ...
  if (gameFile || adminPage) {
    ...
    // Unauthenticated users may access the game file for sector 1 (free-to-try)
    const requestedLevel = request.nextUrl.searchParams.get('autostart') || '1'
    const isSector1 = gameFile && path.endsWith('index.html') && requestedLevel === '1'
    if (!user && !isSector1) return privateResponse(NextResponse.redirect(new URL('/auth/login?redirect=' + (adminPage ? '/admin' : '/'), request.url)))
    if (adminPage && !isGameAdmin(user)) return privateResponse(new NextResponse('Admin access required', { status: 403 }))
    if (gameFile) {
      const access = user ? await gameAccess(supabase, user) : { allowed: false, isAdmin: false, gameId: null }
